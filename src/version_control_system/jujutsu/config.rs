...
impl<'repo> ConfigScope<'repo> {
    fn new_config_command(&self) -> Result<JujutsuCommand, Box<dyn Error>> {
        let mut cmd = JujutsuCommand::new()?;

        match self {
            Self::User => cmd.home_as_current_dir(),
            Self::Repo(repo_path) | Self::Workspace(repo_path) => {
                cmd.repository(repo_path)
            }
        }
    ...
    fn get(&self, key: &str) -> Result<Option<String>, Box<dyn Error>> {
        match self.new_config_command()?.arg("get").arg(key).output() {
            Ok(output) -> Ok(output),
            Err(err) -> {
